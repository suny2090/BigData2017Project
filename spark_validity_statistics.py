...
def check_valid_col_1(x):
    '''CMPLNT_NUM: the validity is determined as follow:
...
if __name__ == "__main__":
    ...
    for i in col_li.keys():
        col_li[i].map(lambda x: (x[3], 1)).reduceByKey(add).map(lambda x: '{0}\t{1}'.format(x[0],x[1])).saveAsTextFile('valid_{0}.out'.format(i))
